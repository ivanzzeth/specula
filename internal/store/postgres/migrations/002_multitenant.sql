...
-- Control-plane timestamps are stored as RFC3339 TEXT (matching the ported
-- org/apikey/grant SQL) so the same SQL runs verbatim on PostgreSQL and SQLite.
-- The int64 users table from 001_create_tables is untouched; org/member/repo
-- ids remain TEXT and join to users by email (members) or acl subject string
-- (repos.owner_user_id). All statements are IF NOT EXISTS (idempotent).
