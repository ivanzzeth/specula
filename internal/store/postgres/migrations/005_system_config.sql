...
-- is node-local, so replica B cannot verify a session replica A issued. This is
-- the postgres (multi-replica) dialect, where that property actually matters.
--
-- `key` and `value` are both non-reserved keywords in PostgreSQL, so they need
-- no quoting; the identical unquoted SQL therefore runs on SQLite too.
