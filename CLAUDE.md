...
- Specula listens on ONE port (7733): protocols, `/token`, Admin API, probes and the WebUI. `server.data_plane_addr` is a startup error; use `server.listen_addr`. `integrate` default `--addr` is `https://127.0.0.1:7733`. Passing `http://` against a TLS-only port auto-upgrades to `https://` (avoids hosts.toml HTTP 400). Use `--skip-scheme-probe` only for offline/tests.
