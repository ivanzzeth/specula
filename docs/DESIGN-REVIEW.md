...
| **cosign keyless（公网 Fulcio/Rekor/OIDC）** | ❌ Rekor/Fulcio/TUF CDN 通常被墙 | CN 下**默认不可用** |
| **cosign trusted_root（自建 Fulcio CA，tlog off）** | ✅ 离线可验证书链 + 叶子签名 | 气隙/自建 sigstore：配 `cosign.trusted_root`；**不做**在线 Rekor / OIDC identity |
