...
| OCI | **signed**（需配公钥或 trusted_root，且镜像被签名） | cosign keyed 和/或 trusted_root Fulcio 证书链（关闭 tlog）；否则 consensus/tofu | 在线 Rekor（`tlog:true`）不支持。**keyed 已端到端验证**：真实 `cosign sign --key --tlog-upload=false`。**trusted_root 已用生产 fetcher 验证**：真实 in-process registry 上带 `dev.sigstore.cosign/certificate` 注解的 `.sig` → TierSigned（`TestCosignVerifier_RealFetcher_TrustedRoot_*`）；CA/叶子为合成 Fulcio 替身（气隙、无在线 Fulcio/OIDC）。验证器只作用于镜像 manifest/index |
